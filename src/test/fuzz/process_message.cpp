...
FUZZ_TARGET(process_message, .init = initialize_process_message)
{
    ...
    // When the fuzzed message type is "tx", replace the random body with a
    // valid transaction spending a mature coinbase. No extra FDP reads.
    ...
    if (net_msg.m_type == "tx" && !g_mature_coinbase.empty() && (++cb_index % 2 == 0)) {
